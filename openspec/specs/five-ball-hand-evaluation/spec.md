...
M2 基础得分 SHALL 等于最佳组合参与槽位的数字总和乘以该组合倍率。污染球 SHALL 占用槽位但不得贡献数字和或倍率。没有槽位时 SHALL 不存在可结算组合且预览分为零。

#### Scenario: Pollution does not score
- **WHEN** 球组为红 3、红 4、红 5、蓝 9且最佳组合为红色同色顺
- **THEN** 基础得分 SHALL 为 `(3+4+5) × 12 = 144`，蓝 9 SHALL 不计分

#### Scenario: Single-ball hand
- **WHEN** 球组仅有数字 6 一格
- **THEN** 最佳组合 SHALL 为单球且基础得分 SHALL 为 6
