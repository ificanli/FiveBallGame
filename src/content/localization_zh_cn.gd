...
const TEXT := {
	...
	"action.shoot": "出杆",
	"action.shoot_now": "击球 [Space]",
	"action.nudge_left": "← 微调",
	"action.nudge_right": "微调 →",
	"input.mode.drag": "拉杆",
	"input.mode.fine": "微调",
	"input.mode.toggle": "输入：%s",
	"feedback.drag_ready": "按住球桌拖拽：方向定角度，距离定力度，松开击球",
	"feedback.drag_cancelled": "拖拽太短，未出杆（可右键取消）",
	"feedback.fine_ready": "移动鼠标/←→ 调方向 · 选 1～5 力度 · 点「击球」出杆",
	"controls.drag": "拉杆：按住拖拽瞄准，松开击球 · 右键取消 · 1～5 力度 · Tab 辅助线 · S 结算 · K 保留 · R 重置",
	"controls.fine": "微调：鼠标/←→ 调方向 · 1～5 力度 · 点击/空格击球 · S 结算 · K 保留 · R 重置",
	"action.settle": "结算",
	"action.keep": "保留球组",
	"action.reset": "同局重置",
	...
	"feedback.tool.soft_pocket": "软袋已就位：五槽满时第六球转为废球",
	"feedback.tool.insurance_slot": "保险槽已就位：可临时六槽，随后强制结算",
	"feedback.tool.color_chalk": "已把本手球染成 %s",
	"feedback.tool.number_sticker": "已调整本手球数字",
	"feedback.tool.return_hook": "已把一颗本手球移出球组",
	"feedback.tool.table_reset": "球桌已重摆，保留分数与杆数",
	"feedback.tool.no_physical_target": "当前球组没有可操作的实体球（被复制的球无法操作）",
	"tool.when.aiming": "出杆前",
	"tool.when.post": "球停后",
